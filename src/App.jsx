...
import myBudjetLogo from "./assets/my-budjet-logo.png"
...
function getPinnedTrackedYears(currentYear, years = []) {
  return [...new Set([currentYear + 1, currentYear, ...years])].sort(
    (left, right) => right - left
  )
}

function getDefaultTrackedYears(currentYear = getCurrentMonthContext().year) {
  return getPinnedTrackedYears(currentYear)
}
...
function getInitialTrackedYears() {
  ...
  try {
    ...
    return normalized.length
      ? getPinnedTrackedYears(currentYear, normalized)
      : getDefaultTrackedYears(currentYear)
  } catch {
    return getDefaultTrackedYears(currentYear)
  }
}
  ...
  try {
    ...
    if (!savedValue) {
      return Object.fromEntries(
        fallbackYears.map((year) => [
          year,
          year === currentYear ? true : year === nextYear ? false : false,
        ])
      )
    }
    ...
    return Object.fromEntries(
      fallbackYears.map((year) => [
        ...
        year === nextYear
          ? false
          : Object.prototype.hasOwnProperty.call(parsed, year)
            ? Boolean(parsed[year])
            : year === currentYear,
      ])
    )
...
function getCashflowAssetTypeLabel(assetType) {
  return (
    cashflowAssetTypeOptions.find((option) => option.value === assetType)
      ?.label ?? assetType
  )
}
...
function buildProjectedExpenseFromTemplate(template, monthContext) {
  ...
  return {
    ...
    effectiveChangeNote:
      effectiveChange && effectiveChange.previousAmount !== effectiveChange.amount
        ? {
            effectiveMonth: monthContext.monthName,
            effectiveYear: monthContext.year,
            previousAmount: effectiveChange.previousAmount,
          }
        : null,
...
function getCashflowSnapshotBalanceKey(snapshot) {
...
function getCashflowSnapshotMonthChange({ snapshot, snapshots }) {
  const previousSnapshot = getPreviousMonthSnapshotForBalance({
    snapshot,
    snapshots,
  })
  if (!previousSnapshot) {
  ...
  const currentAmount = Number(snapshot.amountCzk ?? 0)
  const previousAmount = Number(previousSnapshot.amountCzk ?? 0)
  ...
  return {
    ...
    percent:
      previousAmount > 0
        ? Math.round((Math.abs(delta) / previousAmount) * 100)
        : null,
  }
}
...
function getCurrentBalanceEntries({ snapshots, fundId = null }) {
  ...
  snapshots
    ...
    .forEach((snapshot) => {
      const key = getCashflowSnapshotBalanceKey(snapshot)
  ...
  templates.filter(isValidRegularTemplate).forEach((template, index) => {
    ...
    grouped.set(key, {
      ...
      stoppedFromYear:
        ...
        template.stoppedFromYear ??
        null,
      stoppedFromMonth:
        preferred.stoppedFromMonth ??
...
const monthIncomeLabelMap = {
  Январь: "Доход в январе",
  Февраль: "Доход в феврале",
  Март: "Доход в марте",
  Апрель: "Доход в апреле",
  Май: "Доход в мае",
  Июнь: "Доход в июне",
  Июль: "Доход в июле",
  Август: "Доход в августе",
  Сентябрь: "Доход в сентябре",
...
function MonthFinancialBoard({ monthLabel, monthMeta, summary }) {
  ...
  return (
    <section className="card month-financial-card">
      <div className="month-financial-overview">
        ...
        <div className="month-financial-budget-head">
          <strong>{formatCurrency(summary.incomeTotal)} Kč</strong>
          <span>{incomeLabel}</span>
        </div>
      </div>

      {summary.hasIncomeData ? (
        <>
          <div className="month-financial-section month-financial-section-accent">
            <div className="month-financial-pair month-financial-pair-values month-financial-pair-values-compact">
              <div className="month-financial-pair-item">
                <strong className="month-financial-primary-number">
                  {formatCurrency(summary.totalBudget)} Kč
                </strong>
              </div>
              <div className="month-financial-pair-item month-financial-pair-item-end">
                <strong
                  className={`month-financial-primary-number ${summary.profit < 0 ? "danger" : ""}`}
                >
                  {summary.profit >= 0 ? "+" : "-"}
                  {formatCurrency(Math.abs(summary.profit))} Kč
                </strong>
              </div>
            </div>

            <div className="month-financial-pair month-financial-pair-labels">
              <div className="month-financial-pair-item">
            ...
            >
              ...
              {profitShare > 0 ? (
                <div
                  className="month-financial-bar-segment month-financial-bar-segment-profit"
          ...
          <div className="month-financial-section">
            <div className="month-financial-pair month-financial-pair-values month-financial-pair-values-compact">
              <div className="month-financial-pair-item">
                <strong>{formatCurrency(summary.baseBudget)} Kč</strong>
              </div>
              <div className="month-financial-pair-item month-financial-pair-item-end">
                <strong>{formatCurrency(summary.addedBudget)} Kč</strong>
              </div>
            </div>

            <div className="month-financial-pair month-financial-pair-labels">
              <div className="month-financial-pair-item">
                <span>Регулярные траты</span>
              </div>
              <div className="month-financial-pair-item month-financial-pair-item-end">
                <span>Добавленные траты</span>
              </div>
            </div>
            ...
            >
              ...
              {addedBudgetShare > 0 ? (
                <div
                  className="month-financial-bar-segment month-financial-bar-segment-added"
                  style={{ width: `${addedBudgetShare}%` }}
                />
              ) : null}
            </div>
          </div>
          ...
          <div className="month-financial-section">
            <div className="month-financial-pair month-financial-pair-values month-financial-pair-values-compact">
              <div className="month-financial-pair-item">
                <strong>{formatCurrency(summary.romaPayerBudget)} Kč</strong>
              </div>
              <div className="month-financial-pair-item month-financial-pair-item-end">
                <strong>{formatCurrency(summary.sashaPayerBudget)} Kč</strong>
              </div>
            </div>

            <div className="month-financial-pair month-financial-pair-labels">
              <div className="month-financial-pair-item">
                <span>Оплачивает Рома</span>
              </div>
              <div className="month-financial-pair-item month-financial-pair-item-end">
                <span>Оплачивает Саша</span>
              </div>
            </div>
            ...
            >
              ...
              {sashaPayerShare > 0 ? (
                <div
                  className="month-financial-bar-segment month-financial-bar-segment-payer-sasha"
                  style={{ width: `${sashaPayerShare}%` }}
                />
              ) : null}
            </div>
          </div>
          ...
          <div className="month-financial-section">
            <div className="month-financial-triplet month-financial-triplet-values">
              <div className="month-financial-triplet-item">
                <strong>{formatCurrency(summary.romaOwnerBudget)} Kč</strong>
              </div>
              <div className="month-financial-triplet-item month-financial-triplet-item-center">
                <strong>{formatCurrency(summary.sharedOwnerBudget)} Kč</strong>
              </div>
              <div className="month-financial-triplet-item month-financial-triplet-item-end">
                <strong>{formatCurrency(summary.sashaOwnerBudget)} Kč</strong>
              </div>
            </div>

            <div className="month-financial-triplet month-financial-triplet-labels">
              <div className="month-financial-triplet-item">
                <span>Ромино</span>
              </div>
              <div className="month-financial-triplet-item month-financial-triplet-item-center">
            ...
            >
              ...
              {sharedOwnerShare > 0 ? (
                <div
                  className="month-financial-bar-segment month-financial-bar-segment-owner-shared"
                  style={{ width: `${sharedOwnerShare}%` }}
                />
              ) : null}
              {sashaOwnerShare > 0 ? (
                <div
                  className="month-financial-bar-segment month-financial-bar-segment-owner-sasha"
                  style={{ width: `${sashaOwnerShare}%` }}
                />
              ) : null}
            </div>
          </div>
...
function MobileMonthSwitcher({
  ...
  return (
    <div className={`mobile-month-switcher ${className}`.trim()}>
      ...
      <button
        className="mobile-month-switcher-button mobile-month-switcher-button-center"
        type="button"
      ...
      <button
        className="mobile-month-switcher-button mobile-month-switcher-button-next"
        type="button"
    ...
    [
      {
        ...
        shortLabel: "Регулярные",
        amount: totals.regular,
        color: "#4e9b72",
        icon: "↻",
      },
      {
        key: "irregular",
        label: "Нерегулярные траты",
        shortLabel: "Нерегулярные",
        amount: totals.irregular,
        color: "#e6c27e",
        icon: "＋",
      },
    ],
    ...
    <section className="card analytics-ring-card">
      ...
      {normalizedItems.length ? (
        <div className="analytics-ring-layout">
          <div className="analytics-ring-visual">
            <div className="analytics-ring-chart">
              <svg
                className="analytics-ring-svg"
                viewBox="0 0 100 100"
                aria-label={`${title}: круговая диаграмма`}
              >
                <circle
              ...
              <div className="analytics-ring-hole">
                <strong>{formatCurrency(resolvedTotal)} Kč</strong>
                <span>за год</span>
              </div>
            </div>
            {activeItem ? (
              ...
              >
                ...
                <span>{Math.round(activeItem.share)}%</span>
                <span>{formatCurrency(activeItem.amount)} Kč</span>
              </div>
            ) : null}
          </div>

          <div className="category-breakdown-list analytics-ring-legend">
            {normalizedItems.map((item) => (
              <div
...
function BudgetDataRow({
  ...
  const displayDueLabel = compactDueLabel || dueLabel
    ...
    >
      ...
      >
        <span className="budget-data-main">
          <span className="budget-data-icon-shell">
            <span
            ...
            {showRecurringBadge ? (
              <span className="budget-data-recurring-badge" aria-hidden="true">
                <svg
                  className="budget-data-recurring-icon"
                  viewBox="0 0 20 20"
                  fill="none"
                >
                  <path
                    ...
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M10 3.25a6.75 6.75 0 1 0 6.45 8.76 1 1 0 1 0-1.9-.62A4.75 4.75 0 1 1 10 5.25h2.3L10.9 6.6a1 1 0 1 0 1.4 1.42l3.2-3.14a1 1 0 0 0 0-1.42L12.3.32a1 1 0 1 0-1.4 1.42l1.4 1.51H10Z"
                  />
                </svg>
              </span>
            ) : null}
        ...
        <span className="budget-data-side">
          ...
          {metaAside ? (
            <span className="budget-data-side-meta">{metaAside}</span>
          ) : secondaryLabel ? (
            <span
              className={
                secondaryTone === "muted"
...
function ExpenseEditorModal({ item, onClose, onSave, categories }) {
  ...
  const hasChanges =
    ...
    form.frequency !== resolvedFrequency ||
    form.dayOfMonth !== resolvedDayOfMonth ||
    ...
    <>
      ...
      <section className="card settings-screen-card">
        <div className="settings-stack">
          ...
          >
            <span className="settings-action-title">Фонды сбережений</span>
            <span className="settings-action-note">
      ...
      <header className="page-header">
        <Breadcrumbs
          items={[
            ...
            { label: "Счета сбережений", current: true },
          ]}
        />
        <p className="eyebrow">Сбережения</p>
        <h1>Счета сбережений</h1>
        <p className="page-note">
    ...
    <>
      <header className="page-header">
        <Breadcrumbs
          items={[
            { label: "Настройки", onClick: onBackToSettings },
            { label: "Фонды сбережений", current: true },
          ]}
        />
        <p className="eyebrow">Сбережения</p>
        <h1>Фонды сбережений</h1>
        <p className="page-note">
  ...
  return (
    <section className="card year-card">
      <div className="year-card-head">
        <div className="year-card-summary">
...
function BudgetHubRecurringIcon() {
  return (
    <svg
      viewBox="0 0 32 32"
      aria-hidden="true"
      focusable="false"
      className="budget-hub-glyph"
    >
      <path
        d="M16 4.5a11.5 11.5 0 0 1 8.4 3.6"
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
      ...
      />
      <path
        d="M16 27.5a11.5 11.5 0 0 1-8.4-3.6"
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
        strokeLinejoin="round"
      ...
      />
      <path
        d="M7.1 12.3A11.4 11.4 0 0 1 11.8 6"
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
        strokeLinejoin="round"
      ...
      />
      <path
        d="M24.9 19.7a11.4 11.4 0 0 1-4.7 6.3"
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
        strokeLinejoin="round"
...
function MonthsScreen({
  ...
  return (
    <>
      ...
      <section className="budget-hub-card" aria-label="Быстрые переходы">
        <div className="budget-hub-actions">
          <button
          ...
          >
            <span
              className="budget-hub-action-icon budget-hub-action-icon-month"
              aria-hidden="true"
            >
          ...
          >
            <span className="budget-hub-action-icon" aria-hidden="true">
              <BudgetHubRecurringIcon />
            </span>
            <span className="budget-hub-action-copy">
              <strong>Повторяющиеся</strong>
              <small>Регулярные траты</small>
            </span>
          </button>
      ...
      <div className="years-stack" id="budget-years-stack">
...
function formatRegularSchedule(template) {
  const parsedDay = Number(template.dayOfMonth)
  const dayLabel =
    Number.isFinite(parsedDay) && parsedDay > 0 ? parsedDay : template.dayOfMonth

  if (template.frequency === "Раз в год") {
    const normalizedMonthName = normalizeMonthName(template.month)
    const monthIndex = fullMonthNames.findIndex(
      (item) => item === normalizedMonthName
    )
    const monthLabel =
      monthNamesGenitive[monthIndex] ?? String(template.month ?? "").toLowerCase()

    return `Каждое ${dayLabel} ${monthLabel}`
  }

  return `Каждое ${dayLabel} число месяца`
}

function sanitizeTabularValue(value) {
  return String(value ?? "")
    .replaceAll("\t", " ")
    .replaceAll("\r", " ")
    .replaceAll("\n", " ")
...
function formatRegularExpenseStatus(template, currentMonthContext) {
  return isRegularTemplateScheduledToStop(template)
    ? getRegularStopNote(template, currentMonthContext) ?? "Остановлена"
    : "Активна"
}

function formatRegularExpenseUpcomingChange(template, currentMonthContext) {
  const upcomingChange = getUpcomingRegularChange(template, currentMonthContext)
  if (!upcomingChange) {
    return ""
  }

  return `С ${formatMonthYearLabel(upcomingChange.effectiveMonth, upcomingChange.effectiveYear)}: ${formatCurrency(upcomingChange.amount)} Kč вместо ${formatCurrency(upcomingChange.previousAmount)} Kč`
}

function renderAmountChangeNote({
  previousAmount,
}) {
  return (
    <span className="budget-data-change-hint">
      <span className="budget-data-change-label">Смена цены:</span>
      <span className="budget-data-change-previous">
        <s>{formatCurrency(previousAmount)} Kč</s>
      </span>
    </span>
  )
}

function formatRegularExpenseHistory(template) {
  if (!template.history?.length) {
    return ""
  }
  ...
  return [...template.history]
    ...
    .map(
      (entry) =>
        `${formatMonthYearLabel(entry.effectiveMonth, entry.effectiveYear)}: ${formatCurrency(entry.amount)} Kč`
    )
    .join(" | ")
}

function buildRegularExpensesTsv({
  regularExpenses,
  categories,
  currentMonthContext,
}) {
  ...
  const rows = regularExpenses
    .filter((template) => template.isActive !== false)
    .sort((left, right) => {
      const ownerCompare = String(left.owner ?? "").localeCompare(
        String(right.owner ?? ""),
        "ru"
      )
      if (ownerCompare !== 0) {
    ...
    .map((template) => {
      ...
      return [
        ...
        currentAmount,
        template.frequency,
        template.frequency === "Раз в год" ? template.month : "",
        template.dayOfMonth,
        formatRegularSchedule(template),
        template.cadence === "auto" ? "Автоплатеж" : "Ручной платеж",
        template.owner,
        template.payer,
        categoryMeta.label,
        formatRegularExpenseStatus(template, currentMonthContext),
        formatRegularExpenseUpcomingChange(template, currentMonthContext),
        formatRegularExpenseHistory(template),
      ].map(sanitizeTabularValue)
    })

  return [headers, ...rows].map((row) => row.join("\t")).join("\n")
}

function downloadRegularExpensesFile({
  regularExpenses,
  categories,
  currentMonthContext,
}) {
  ...
  const tsv = buildRegularExpensesTsv({
    regularExpenses,
    categories,
    currentMonthContext,
  })
  const exportDate = new Date().toISOString().slice(0, 10)
  const fileName = `regular-expenses-${exportDate}.tsv`
        ...
        <>
          <div className="analytics-grid">
            ...
            <section className="card annual-outcome-card">
              ...
              <div className="category-breakdown-head">
                <h2>Доходы, расходы и прибыль</h2>
              </div>

              {hasAnyIncomeData ? (
                <>
                  <div className="annual-outcome-income-bar">
          ...
          <div className="analytics-grid analytics-grid-secondary">
            <AnalyticsRingCard
              eyebrow="Расходы по владельцу"
              title={`Общие, Ромины и Сашины траты за ${selectedYear} год`}
              items={ownerBreakdown}
              note="Категория OSVC здесь не учитывается: эти расходы встроены в доход и не участвуют в сравнении общих и личных трат."
              emptyText={`Пока нет трат за ${selectedYear} год, чтобы показать деление на общие и личные расходы.`}
            />

            <AnalyticsRingCard
              eyebrow="Регулярность расходов"
              title={`Регулярные и нерегулярные траты за ${selectedYear} год`}
              items={regularityBreakdown}
              note="Категория OSVC здесь не учитывается: эти расходы встроены в доход и не участвуют в сравнении регулярных и добавленных трат."
              emptyText={`Пока нет трат за ${selectedYear} год, чтобы сравнить регулярные и добавленные нерегулярные расходы.`}
            />
          </div>
        </>
      ) : (
...
function RegularExpenseTemplateCard({
  ...
  return (
    <BudgetDataRow
      ...
      categoryIcon={categoryMeta.icon}
      categoryLabel={categoryMeta.label}
      note={stopNote ? stopNote : null}
      metaAside={
        !stopNote && upcomingChange
          ? renderAmountChangeNote({
              effectiveMonth: upcomingChange.effectiveMonth,
              effectiveYear: upcomingChange.effectiveYear,
              previousAmount: upcomingChange.previousAmount,
            })
          : null
      }
  ...
  return (
    <>
      ...
      <section className="card regular-summary-card">
        ...
        <div className="regular-summary-actions">
          <button
            className="secondary-action-button"
            type="button"
            onClick={onExport}
      ...
      {ownerGroups.map((group) => (
        <section key={group.owner} className="card regular-group-card">
          ...
          <div className="regular-subgroup">
            ...
            <div className="regular-template-list">
              {group.stopped.length ? (
                group.stopped.map((template) => (
                  <RegularExpenseTemplateCard
                    key={template.id}
                    template={template}
                    categories={categories}
...
function ProjectedExpenseRow({
  ...
  return (
    <BudgetDataRow
      ...
      categoryIcon={categoryMeta.icon}
      categoryLabel={categoryMeta.label}
      note={expense.effectiveChangeNote ? null : expense.effectiveNote ?? null}
      metaAside={
        expense.effectiveChangeNote
          ? renderAmountChangeNote(expense.effectiveChangeNote)
          : null
      }
      amount={expense.amount}
...
function CashflowReserveTargetModal({ fund, onClose, onSave }) {
  ...
  return (
    <ModalShell title={title} onClose={onClose} compact>
      <div className="modal-header">
        <h2>{title}</h2>
        <p className="modal-subtitle">{helperText}</p>
      </div>
      <div className="modal-form">
        <label className="field">
          <span className="field-head">
          ...
          <button
            ...
            onClick={() =>
              onSave({
                ...
                targetMultiplier: isManual ? null : parsedMultiplier,
                manualTargetAmount: isManual ? parsedManualTarget : null,
              })
            }
          >
            Сохранить
...
function CashflowSnapshotRow({ snapshot, onClick }) {
  const change = snapshot.monthChange
  const isPositiveChange = Number(change?.delta ?? 0) > 0
...
function CashflowSnapshotDetailsModal({
  ...
  const assetTypeLabel = getCashflowAssetTypeLabel(snapshot.assetType)
  ...
  const currentBalanceEntries = getCurrentBalanceEntries({ snapshots }).map(
    (snapshot) => {
      ...
      return {
        ...
        assetTypeLabel: getCashflowAssetTypeLabel(snapshot.assetType),
        monthChange: getCashflowSnapshotMonthChange({ snapshot, snapshots }),
      }
    }
  )
    ...
    <>
      ...
      <div className="cashflow-layout">
        <section className="card cashflow-panel">
          ...
          {prioritizedBalanceEntries.length ? (
            <div className="cashflow-mini-list">
              {prioritizedBalanceEntries.map((snapshot) => (
                <CashflowSnapshotRow
                  key={snapshot.id}
                  snapshot={snapshot}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const updateBackgroundMotion = () => {
      const scrollTop =
        ...
        0
      const appImageOffset = Math.max(-42, -18 - scrollTop * 0.05)
      const mobileHeroOffset = Math.max(-68, -30 - scrollTop * 0.145)
  ...
  return (
    <>
        ...
        >
            ...
            <CashflowAccountsSettingsScreen
              ...
              onRequestDelete={(account) =>
                setCategoryConfirmState({
                  ...
                  description:
                    "Счёт будет удалён из справочника сбережений, если он нигде не используется.",
                  onConfirm: async () => {
            ...
            <CashflowFundsSettingsScreen
              ...
              onRequestDelete={(fund) =>
                setCategoryConfirmState({
                  ...
                  description:
                    "Фонд будет удалён из справочника сбережений, если он нигде не используется.",
                  onConfirm: async () => {
          ...
          {currentScreen === "nextMonth" ? (
            <NextMonthBudgetScreen
              ...
              financialSummary={nextMonthFinancialSummary}
          ...
          {currentScreen === "regular" ? (
            <RegularExpensesScreen
              ...
              onStop={(template) => setRegularStopState({ template })}
              onResume={handleResumeRegularExpense}
            />
          ) : null}
          ...
          {currentScreen === "months" ? (
            <MonthsScreen
              ...
              currentMonthLabel={currentMonthContext.monthName}
          ...
          {currentScreen === "analytics" ? (
            <AnalyticsScreen
              ...
              ownerBreakdown={analyticsYearOwnerBreakdown}
              regularityBreakdown={analyticsYearRegularityBreakdown}
      ...
      {regularStopState ? (
        <RegularExpenseStopModal
          template={regularStopState.template}
          currentMonthContext={currentMonthContext}
          trackedYears={trackedYears}
          onClose={() => setRegularStopState(null)}
          onSave={handleStopRegularExpense}
        />
      ) : null}
